...
exports.getAll = async (req, res) => {
    try {
        ...
        const [totalDocs, results] = await Promise.all([
            Product.find(filter).sort(sort).populate('brand').countDocuments().exec(),
            Product.find(filter).sort(sort).populate('brand').skip(skip).limit(limit).exec(),
        ]);
...
exports.undeleteById = async (req, res) => {
    try {
        const { id } = req.params;
        const unDeleted = await Product.findByIdAndUpdate(id, { isDeleted: false }, { new: true }).populate(
            'brand'
        );
...
exports.deleteById = async (req, res) => {
    try {
        const { id } = req.params;
        const deleted = await Product.findByIdAndUpdate(id, { isDeleted: true }, { new: true }).populate(
            'brand'
        );
